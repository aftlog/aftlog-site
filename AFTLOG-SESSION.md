...
- **🔴 OPEN ITEMS (see Session 4 for full log):**
  ...
  4. **Translations (Rule 3 pass at release):** core strings only → **Tranche 1 DONE 1.103.5** (fuel/trip wizards + add boat: 62 keys × 5 locales + coverage test). Remaining ~217 Text() strings: calculators, boat detail, checklists, more/settings, dashboard, sheets. Tracking: `test/translations_test.dart` asserts every added key resolves in all 5 locales (no EN fallback).
