...
- **🔴 OPEN ITEMS (see Session 4 for full log):**
  ...
  4. **Translations (Rule 3 pass at release):** core strings only → **Tranche 1 DONE 1.103.5** (wizards + add boat: 62 keys) · **Tranche 2 DONE 1.103.6** (calculators: 93) · **Tranche 3 DONE 1.103.7** (boat detail: 58) · **Tranche 4 DONE 1.103.8** (checklists tab + detail/wizards: 58). Remaining: more/settings, dashboard, sheets (~90 Text() strings). Tracking: `test/translations_test.dart` (~271 keys × 5 locales, no EN fallback).
