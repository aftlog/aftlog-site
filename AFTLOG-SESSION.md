...
- **🔴 OPEN ITEMS (see Session 4 for full log):**
  ...
  4. **Translations (Rule 3 pass at release):** core strings only → **Tranche 1 DONE 1.103.5** (fuel/trip wizards + add boat: 62 keys) · **Tranche 2 DONE 1.103.6** (9 calculators: 93 keys) · **Tranche 3 DONE 1.103.7** (boat detail: 58 keys, incl. add-doc/add-part/log-service dialogs). Remaining: checklists, more/settings, dashboard, sheets (~90 Text() strings). Tracking: `test/translations_test.dart` asserts every added key resolves in all 5 locales (~213 keys total, no EN fallback).
