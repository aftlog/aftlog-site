...
- **Open (parked):** ~~hotspot visual alignment~~ ✅ finished (Louis, 08-15) · **Site redesign DONE 08-15** — hybrid dark hero + light content deployed (index.html + aftlog.css); portal links live everywhere with placeholder href (`https://portal.aftlog.com`) — **swap for the real URL when the portal deploys** · deep-screen translations ✅ COMPLETE 1.103.12 · site rollout (analytics → beta CTA → distribution; Aug 24 gate) · MLD logo spacing done 5j.
- **🔴 OPEN ITEMS (see Session 4 for full log):**
  1. ~~Hotspot alignment~~ ✅ FINISHED 2026-08-15 (Louis).
  2. **Review backlog (Session 4 findings, cheap fixes):** ~~daily check-in dies on reboot (`RECEIVE_BOOT_COMPLETED` missing)~~ ✅ fixed (manifest has it + plugin boot receiver verified 08-15) · ~~onboarding flag written before onboarding finishes~~ ✅ fixed 1.50.1 + hardened 1.103.3 (`_pick` now marks onboarded only AFTER the checklist exists; failure resets retry) · free-tier gating mostly enforced (AI/reminders/NBA gated; **#102 shipped 08-15**: Boat Health breakdown, Prop slip/Fuel range calculators, planner completed history) · ~~Boats tab stale (listen to `BoatEvents`)~~ ✅ Boats tab + Dashboard already listened; **1.103.2 fixed the other IndexedStack tabs**: BeginnerDashboardView (never listened — stale primary boat/score), LogScreen (stale selector + deleted-boat guard), ChecklistsScreen (trailer-aware list) · ~~`Units` service dead code (metric toggle half-wired)~~ ✅ toggle wired in 1.50.1; dead helpers `fuelValue`/`speedValue` + `_metric` alias removed in 1.103.4
  ...
  4. **Translations (Rule 3 pass at release):** ✅ **COMPLETE 2026-08-15 (1.103.5–1.103.12)** — all 8 tranches: wizards+addBoat (62), calculators (93), boat detail (58), checklists (58), More/Settings (118), dashboard (33), **analytics sheets (~220)** — ~640 keys × 5 locales, `test/translations_test.dart` guards every key (no EN fallback). Design notes: season/cluster/month names used as DATA KEYS stay English (grouping/lookups); display maps via helpers. Remaining minor: data-level labels (DriveType/HullType/DocType/ChecklistType enum labels) still EN — flagged for a future data-layer decision.
  5. **Pro purchase wiring:** `pro_service.dart` is just a flag — the $29 unlock isn't purchasable yet
  6. ~~Scheduled interval notifications~~ ✅ SHIPPED — feature #72 (Planner notifications: due/due-soon/seasonal/monthly state-transition alerts, quiet hours, prefs dedupe) + daily check-in toggle. Verified end-to-end 08-15 (intent #104).
