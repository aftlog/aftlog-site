...
| **Portal-touching builds MUST carry the dev key — `./build.sh dev`, or say they don't** | BUILD A MANUAL Stage 2 APK was handed over for testing portal features (Link to Portal, draft sync/pair/upload, AI field-fill — all `AFTLOG_DEV_KEY` / dev-flavor gated) and the user could not exercise them (no Link to Portal). For ANY app-side feature that touches portal auth, drafts, pairing, or Cloud Functions, deliver either (1) a `./build.sh dev` APK (carries `AFTLOG_DEV_KEY` + `APP_VERSION=dev`), or (2) an APK explicitly labeled "this APK does NOT contain the dev key — rebuild locally before testing." A plain `flutter build apk --release` without the dev defines hides dev-gated tiles. When in doubt, build via `./build.sh dev` (which wires the defines + release keystore) and confirm the defines/`aapt` before handing over. |
