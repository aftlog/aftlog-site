...
def main():
    ...
    if sm.exists():
        ...
        # Every public page must be listed — not just the main list above
        # (the sitemap is generated by tools/generate_sitemap.py).
        ...
        for f in ROOT.rglob("*.html"):
            ...
            if rel == "index.html":
                ...
                url = "https://aftlog.com/" + (f"{parent}/" if parent in slash_index else rel)
            else:
                url = "https://aftlog.com/" + rel
            if url not in smc:
                missing.append(url)
        check("sitemap covers every public page", not missing,
              f"missing: {missing[:6]}")
